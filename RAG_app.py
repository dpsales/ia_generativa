...
####################################################################
st.set_page_config(page_title="Chat com seus dados")
...
def expander_model_parameters(
    LLM_provider="OpenAI",
    text_input_API_key = os.getenv("OPENAI_API_KEY"),
    list_models=["gpt-4o", "gpt-4o-mini", "gpt-4.1-mini"],
):
    """Set the LLM provider and configure the session state for API keys."""
    ...
    with st.expander("**Modelos e parametros**"):
...
def sidebar_and_documentChooser():
    ...
    with st.sidebar:
        st.caption(
            "🚀 Uma RAG chatbot pelo 🔗 Langchain, OpenAI e Huggingface 🤗"
        )
        st.write("")
        ...
        llm_chooser = st.radio(
            ...
            list_LLM_providers,
            captions=[
                "[OpenAI preços](https://openai.com/pricing)",
                "Limite rate: 60 requisições por minuto.",
                "**Free access.**",
            ],
        )
        ...
        if llm_chooser == list_LLM_providers[1]:
            expander_model_parameters(
                ...
                text_input_API_key= os.getenv("HF_API_KEY"),
                list_models=["mistralai/Mistral-7B-Instruct-v0.2"],
            )
        # Assistant language
        st.write("")
        st.session_state.assistant_language = st.selectbox(
            f"Assistente de língua", list(dict_welcome_message.keys())
        )

        st.divider()
        st.subheader("Recuperadores")
        retrievers = list_retriever_types
        if st.session_state.selected_model == "gpt-4o-mini":
            # for "gpt-4o-mini", we will not use the vectorstore backed retriever
            # there is a high risk of exceeding the max tokens limit (4096).
            retrievers = list_retriever_types[:-1]

        st.session_state.retriever_type = st.selectbox(
            f"Selecione e tipo de Recperador", retrievers
        )
    
        st.write("\n\n")
        st.write(
            f"ℹ _Sua {st.session_state.LLM_provider} API key, '{st.session_state.selected_model}' paramentros, \
            e {st.session_state.retriever_type} are only considered when loading ou criação de um vectorstore._"
        )

    # Tabbed Pane: Create a new Vectorstore | Open a saved Vectorstore

    tab_new_vectorstore, tab_open_vectorstore = st.tabs(
        ["Crie um novo Vectorstore", "Open um Vectorstore salvo"]
    )
    with tab_new_vectorstore:
        ...
        st.session_state.vector_store_name = st.text_input(
            label="🗂️ Nome do seu Vectorstore",
            placeholder="Ex: meus_documentos",
            help="Dê um nome simples para identificar sua base de documentos.",
            max_chars=50
        )
        # 3. Add a button to process documnets and create a Chroma vectorstore

        if st.button("Criar Vetorbases"):
            chain_RAG_blocks()
            try:
    ...
    with tab_open_vectorstore:
        st.write("📂 Carregar um Vectorstore salvo")

        clicked = st.button("🔍 Selecione o Vectorstore")
        root = None
        ...
        # Abrir seletor de pasta
            ...
            if not selected_vectorstore_path:
                st.info("ℹ️ Nenhuma pasta selecionada.")
            else:
                st.session_state.selected_vectorstore_name = os.path.basename(selected_vectorstore_path)
            ...
            # Validar API keys
                if (
                    ...
                    st.warning(f"⚠️ Insira sua {st.session_state.LLM_provider} API key.")
                    return

                with st.spinner("🔄 Carregando vectorstore..."):
                    try:
                    # 1. Carregar embeddings
                        embeddings = select_embeddings_model()

                    # 2. Carregar vectorstore salvo
                        st.session_state.vector_store = Chroma(
                            embedding_function=embeddings,
                            persist_directory=selected_vectorstore_path,
                            collection_name=st.session_state.selected_vectorstore_name,
                    ...
                    # 4. Criar ConversationalRetrievalChain
                        (
                    ...
                    # 5. Limpar histórico
                        ...
                        st.success(f"✅ Vectorstore **{st.session_state.selected_vectorstore_name}** carregado com sucesso!")
...
def select_embeddings_model():
    """Selecione um modelo de Embeddings: OpenAIEmbeddings."""
...
def chain_RAG_blocks():
    st.write("🔧 Iniciando criação do vectorstore...")
    st.write("📎 Arquivos:", st.session_state.get("uploaded_file_list"))
    st.write("📛 Nome do vectorstore:", st.session_state.get("vector_store_name"))
    ...
    if (
        ...
        error_messages.append(
            f"inserir sua {st.session_state.LLM_provider} API key"
        )

    if not st.session_state.uploaded_file_list:
        error_messages.append("selecionar documentos para upload")

    if not st.session_state.vector_store_name:
        error_messages.append("preencher o nome do Vectorstore")

    if error_messages:
        mensagem = "⚠️ Favor " + ", ".join(error_messages[:-1])
        if len(error_messages) > 1:
            mensagem += " e " + error_messages[-1]
        else:
            mensagem += error_messages[0]
        st.warning(mensagem + ".")
        return
    ...
    try:
        ...
        for uploaded_file in st.session_state.uploaded_file_list:
            try:
                ...
                error_message += f"\nErro ao salvar {uploaded_file.name}: {str(e)}"

        if error_message:
            st.warning(f"⚠️ Problemas ao salvar arquivos: {error_message}")
            return
        ...
        st.success(
            f"✅ Vectorstore **{st.session_state.vector_store_name}** criado com sucesso."
        )
        ...
        try:
            ...
            st.success("✅ Chain e memória criadas com sucesso!")
        except Exception as e:
            st.error(f"❌ Erro ao criar a chain: {str(e)}")
            return

        # 9. Limpar histórico do chat
        clear_chat_history()

    except Exception as error:
        st.error(f"❌ Ocorreu um erro inesperado: {str(error)}")
...
def create_memory(model_name="gpt-4o-mini", memory_max_token=None):
    """Creates a ConversationSummaryBufferMemory for gpt-4o-mini
    Creates a ConversationBufferMemory for the other models"""

    if model_name == "gpt-4o-mini":
        if memory_max_token is None:
            memory_max_token = 1024  # max_tokens for 'gpt-4o-mini' = 4096
...
def answer_template(language="portuguese"):
...
def create_ConversationalRetrievalChain(
    ...
    condense_question_prompt = PromptTemplate(
        ...
        template="""Considerando a seguinte conversa e uma pergunta complementar,
        reformule a pergunta complementar para que seja uma pergunta independente, em seu idioma original.\n\n
        Histórico do Chat:\n{chat_history}\n
        Entrada Complementar: {question}\n
    ...
    # 3. Add ConversationSummaryBufferMemory for gpt-4o-mini, and ConversationBufferMemory for the other models
...
def chatbot():
    ...
    with col2:
        st.button("Limpar o Chat", on_click=clear_chat_history)
    ...
    if prompt := st.chat_input():
        ...
        if "chain" not in st.session_state:
            st.warning("Por favor, crie ou carregue um vectorstore antes de iniciar o chat.")
            st.stop()
        # Check inputs        
        if (
            not st.session_state.openai_api_key
            ...
            st.info(
                f"Inserira sua {st.session_state.LLM_provider} API key para continuar."
            )
            st.stop()
